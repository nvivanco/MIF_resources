    ...
    input_ch = config_csv_ch.csv
        ...
        .map { row ->
            def meta = [
                ...
                diameter         : row.diameter != null && row.diameter != '' ? row.diameter as int : null,
                niter            : row.niter != null && row.niter != '' ? row.niter as int : null,
    ...
    ch_cellpose_inputs = TILE_CONSTRUCTOR.out.tiles
        ...
        .flatMap { meta, tiles_csv, raw_zarr ->
            tiles_csv.splitCsv(header: true).collect { row ->
                ...
                tile_meta.diameter = row.diameter != null && row.diameter != '' ? row.diameter as int : null
                tile_meta.niter = row.niter != null && row.niter != '' ? row.niter as int : null
